...
2026-04-25T04:50 — Fase 3 concluída. Commit 6f2c4a5: timeline com drag-drop em qualquer área do form, botões "anexar imagem" e "anexar arquivo" com inputs hidden, pending-files como chips removíveis, submit cria comment + sobe anexos sequencialmente (limite 25MB cada), CommentItem renderiza imagens em grade 1-2 colunas e outros arquivos como chips com download/remover, suporte a placeholder "[anexo]" quando comment só tem anexos. Helpers novos em queries/cards.ts: presignAttachmentForComment + createAttachmentForComment + uploadAttachmentForComment usando endpoints /comments/:id/attachments[/presign] da Fase 1. Tipo CommentNode ganhou attachments?: Attachment[]. Typecheck e lint verdes. WhatsApp enviado. Registro de 04:50.

2026-04-25T04:50 — Operador pediu pra finalizar a tarefa atual e não começar a próxima. Etapas 3-7 do doc 18 (cronômetro) ficam pendentes pra outra sessão (ou retomada via cron quando heartbeat envelhecer). Estado pra retomada: Fases 1-3 dos anexos concluídas (commits bf047cc, 7ababde, fe74529, 6f2c4a5). Próximo escopo: doc 18 etapas 3 (bloco Tempo registrado no modal) → 4 (página /indicadores/timesheet) → 5 (resumo Membros) → 6 (realtime + menu) → 7 (CSV + polish), depois checklist.md em fallback. Todos os processos encerrados. Registro de 04:50.
