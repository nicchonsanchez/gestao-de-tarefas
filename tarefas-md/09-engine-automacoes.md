...
type Condition =
  ...
  | {
      field: `custom.${string}`;
      op: 'eq' | 'neq' | 'gt' | 'lt' | 'contains' | 'isEmpty';
      value?: unknown;
    }
...
type Action =
  ...
  | {
      id: string;
      type: 'SET_DUE_DATE';
      params: { daysFromNow: number; time?: 'HH:mm'; businessDays?: boolean };
    }
  ...
  | {
      id: string;
      type: 'SEND_EMAIL';
      params: { to: EmailRecipient; templateId: string; data?: Record<string, unknown> };
    }
  | {
      id: string;
      type: 'SEND_WHATSAPP';
      params: { integrationId: string; to: WhatsRecipient; templateId: string };
    }
  | {
      id: string;
      type: 'CREATE_CARD';
      params: { boardId: string; listId: string; title: string; fromTemplateId?: string };
    }
  | {
      id: string;
      type: 'CALL_WEBHOOK';
  ...
  | {
      id: string;
      type: 'DELAY';
      params: { minutes?: number; hours?: number; days?: number; untilBusinessHours?: boolean };
    }
  | {
      id: string;
      type: 'IF';
      params: { condition: Conditions; thenActions: Action[]; elseActions?: Action[] };
    }; // v2
...
```ts
@Injectable()
export class MoveToListHandler implements ActionHandler<{
  listId: string;
  position?: 'top' | 'bottom';
}> {
